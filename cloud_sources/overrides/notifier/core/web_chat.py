...
async def verify_chat_recipient_uid(page, expected_uid: str | None) -> None:
    """Verify the profile opened by THIS chat header, never by list/search text.

    The header avatar and /user/<sec_uid> popup were observed on the live web
    chat. No profile content, cookies, or private framework state is inspected.
    A layout change or blocked popup fails closed; there is no name fallback.
    """
    popup = None
    try:
        ...
        if await editor.count() != 1 or not await editor.is_visible():
            raise RecipientIdentityError()
        async with page.expect_popup(timeout=8000) as opened:
            await header.click(timeout=5000)
        popup = await opened.value
        await popup.wait_for_url('https://www.douyin.com/user/*', wait_until='commit', timeout=8000)
        profile = urlparse(popup.url)
        if (profile.scheme != 'https' or profile.netloc != 'www.douyin.com'
                or profile.path != '/user/' + expected_uid):
            raise RecipientIdentityError()
        ...
        if popup is not None:
            try:
                ...
                raise RecipientIdentityError('身份核验页面未能安全关闭，已停止发送') from None
