...
def create_app(settings: Settings, engine: Engine) -> FastAPI:
    ...
    def admin_monthly_slots(request: Request, user_id: str, csrf_token: str = Form(default=""),
        ...
        with task_write_lock, session_scope(engine) as db:
            ...
            auth.csrf(record, csrf_token)
            try:
                TaskCapacityService(db, AuditService(db)).purchase_monthly(admin.id, user_id, count)
    ...
    def admin_renew_monthly(request: Request, grant_id: str, csrf_token: str = Form(default="")):
        with task_write_lock, session_scope(engine) as db:
            admin, record, _ = auth.admin_context(request, db)
            auth.csrf(record, csrf_token)
            try:
                grant = TaskCapacityService(db, AuditService(db)).renew_monthly(admin.id, grant_id)
            except (ValidationError, NotFound) as error:
                raise HTTPException(400, str(error)) from error
    ...
    def admin_retry_run(
        ...
        with task_write_lock, session_scope(engine) as db:
            ...
            batch_rows = BatchRunService(db).rows(run.id) if run else []
            safe_retry = any(row.retryable for row in batch_rows) if batch_rows else bool(
                run and run.error_code in SAFE_MANUAL_RETRY_CODES)
            if (
                run
                and task
                and task.enabled
                and run.status in ({"failed", "partial"} if batch_rows else {"failed"})
                and safe_retry
                ...
                if already_scheduled:
                    ...
                    if batch_rows:
                        BatchRunService(db).remember_retry(task.id, run.id, task.next_run_at)
                    AuditService(db).write(
                        admin.id,
