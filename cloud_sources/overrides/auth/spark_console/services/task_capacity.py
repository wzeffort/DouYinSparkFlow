...
class TaskCapacityService:
    ...
    def purchase_monthly(self, actor_id, user_id, count, at=None):
        if not isinstance(count, int) or not 1 <= count <= 100:
            raise ValidationError("月卡名额数量须为 1–100")
        now = self._aware(at or datetime.now(timezone.utc))
        return [self.grant(actor_id, user_id, 1, now, now + timedelta(days=30), "30 天任务月卡")
    ...
    def renew_monthly(self, actor_id, grant_id, at=None):
        ...
        if grant.amount != 1 or grant.expires_at is None or grant.revoked_at is not None:
            raise ValidationError("仅支持对未撤销的独立限时名额续期")
        now = self._aware(at or datetime.now(timezone.utc))
        grant.expires_at = max(now, self._aware(grant.expires_at)) + timedelta(days=30)
        self.session.flush()
        self.audit.write(actor_id, "quota.monthly_renewed", "task_quota_grant", grant.id)
        return grant
